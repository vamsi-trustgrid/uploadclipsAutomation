...
const USERNAME = "ylovingly35@gmail.com";
const PASSWORD = "yourslovingly";
const REELS_TO_COLLECT = 1; // change to 20 when ready
const DOWNLOADER_URL = "https://fastdl.app/en2";
// ─────────────────────────────────────────────────────────────────────────────
...
async function downloadReels(links) {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    // Paste link directly into input via native setter (no char-by-char typing)
    await wc.executeJavaScript(`
      (function() {
        const input = document.querySelector("#search-form-input");
        if (!input) return;
        const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;
        nativeSetter.call(input, ${JSON.stringify(link)});
        input.dispatchEvent(new Event("input", { bubbles: true }));
        input.dispatchEvent(new Event("change", { bubbles: true }));
        input.focus();
        console.log("[Downloader] Link pasted into input.");
      })();
    `);
    await sleep(500);
...
async function main(wc) {
  ...
  // Close Instagram window before opening downloader
  // if (mainWindow && !mainWindow.isDestroyed()) {
  //   console.log("[Main] Closing Instagram window...");
  //   mainWindow.close();
  //   mainWindow = null;
